 ...
 * @date   Fri Aug  1 15:29:34 2003
 * 
 * @brief This file contains the definitions of the GenericValues
 * class which makes it possible to map values related to textual
...
/**
 * Constructor for the GenericValueT class. It creates a GenericValueT
 * object by setting all the private member variables to the values
...
{
  m_strName  = strdup( str ); // strdup needed to allocate right amount of memory
  m_vAddress = vAddr;
  m_type     = t;
}
...
/**
 * Destructor for the GenericValueT class. It destroys a GenericValueT
 * object by freeing all memory allocated to it. */
 ...
 *
 * @param strValue a string denoting the value to which the variable associated
 *  with this GenericValueT object must be set
 * @return a boolean indicating whether the update was successful 
 */
bool GenericValueT::setValue( char *strValue )
{
  bool bReturn = true, b = false;

  int i = strlen( strValue ) - 1; 
	while( i >= 0 && isspace( strValue[i] ) )
	  strValue[i] = '\0';
 ...
 * of the variable associated with this GenericValueT object
 * @return a string containing the value of the variable associated with this
 *  GenericValueT object. 
 */
...
/**
 * This display method writes the name associated with the variable in
 * this GenericValueT object together with its value to the output
 ...
 * @param strSeparator a string representing the separator which should be
 * written between the name associated with the variable and its value 
 */
...
/**
 * Constructor for the GenericValues class. It creates a GenericValues
 * object.
 * 
...
/**
 * Destructor for the GenericValues class. It destroys a GenericValues
 * object by freeing all memory allocated to it. 
 */
 ...
 *
 * @return the total number of generic values stored in the collection so far
 */
...
/**
 * This method adds a new generic value to the collection and provides
 * a connection between a string name and the variable name of the
 * generic value.
 *
 * @param strName a string denoting the name associated with the variable of
 * the generic value which is added (the variable can be reached through this
 * name)
...
/**
 * This (private) method returns a pointer to the GenericValueT object
 * of which the name associated with the variable matches the argument
 * passed to the method.
 *
 * @param strName a string denoting the name associated with the variable of
 * the GenericValueT object to which a pointer should be returned
 * @return a pointer to the GenericValueT object of which the name associated
 * with the variable matches the argument; 0 when a GenericValueT object with a
 * variable associated with the given name does not exist 
 */
 ...
 * associated with it matches the first argument to the method. The
 * value is converted to a string which is put into the second
 * argument (note that enough memory must be allocated for this
 * char*). This same string is also returned by the method. This
 * enables you to use the method as an argument to a function such as
 * 'strcpy'.
 *
 * @param strName a string denoting the name associated with the variable of
 ...
 * the second argument. The value is always supplied as a string which
 * is then converted into the right type for the generic value in
 * question.
 *
 * @param strName a string denoting the name associated with the variable which
 * must be set
 * @param strValue a string denoting the value to which the variable indicated
 *  by the first argument should be set
 * @return a boolean indicating whether the update was successful 
 */
bool GenericValues::setValue( const char *strName, char *strValue )
{
  bool bReturn = false;
  GenericValueT *parptr;
  ...
  if( parptr )
    bReturn = parptr->setValue( strValue ); // string is converted to right type

  return ( bReturn );
}

/**
 * This method reads generic values from a file indicated by the first
 * argument to the method and stores them in the collection. In this
 * file the names associated with each value must be listed first
 * followed by a separator and then the value. The names associated
 * with each value should match the string names with which the
 * corresponding generic value is added using 'addSetting'.
 *
  ...
  while( in.getline( strLine, sizeof( strLine ) ) )
  {
             ...
             strLine[ 1 ] == '#' ) ) )
    {
      ...
      if( sscanf( strLine, "%s%s", strName, strValue ) != 2 )
			...
			{
			  char *tmp = strchr( strValue, '#' );
				if( tmp != NULL )
				  *tmp = '\0';
        bReturn = setValue( strName, strValue );
      }
 ...
 *
 * @param strFile a string denoting the name of the file to which the generic
 * values should be written
 * @param strSeparator a string representing the separator which should be
 * written between the name associated with each value and the value itself
 * @param bAppend a boolean indicating whether the values should be appended to
 * the given file (=true) or if current contents of the file should be
 * overwritten (=false)
 * @return a boolean indicating whether the values in the current collection
 * have been correctly written to the given file 
 */
 ...
 * second argument.m
 *
 * @param out the output stream to which the current collection of generic
 * values should be written
 * @param strSeparator a string representing the separator which should be
 * written between the name associated with each value and the value itself 
 */
